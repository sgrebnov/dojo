...
function(dojo, aspect, dom, domClass, lang, on, has, mouse, domReady, win){
	...
	var hasTouch = has("touch"),
		...
		normalizeEvent = function (originalEvent, eventType, preferredEventName) {
			...
			var pointerProperties = {"width" : 0, "height" : 0, "pressure" : 0, "tiltX" : 0, "tiltY" : 0, 
				...
				if (eventType === POINTER_TYPE_MOUSE) {// mouse event
					...
					normalizedEvents.push(makePointerEvent(originalEvent, pointerProperties)); 
					...
					if(!originalEvent.changedTouches) { // special case for dojo specific emulated events like dojotouchover
						...
						normalizedEvents.push(makePointerEvent(originalEvent, lang.mixin(touchProperties, pointerProperties)));
					...
					for(var i=0; i<originalEvent.changedTouches.length; i++) {
						...
						normalizedEvents.push(makePointerEvent(originalEvent, pointerPropertiesForTouch));
					};                    
				}
			return normalizedEvents;
		},
		 ...
		 */
		makePointerEvent = function(originalEvent, properties) {
	...
	var pointerModule = has("pointer"),
		pointer = {
		down: bindEvent("PointerDown","mousedown", "touchstart", pointerModule + "Down"),
		move: bindEvent("PointerMove", "mousemove", "touchmove", pointerModule + "Move"),
		up: bindEvent("PointerUp", "mouseup", "dojotouchend", pointerModule + "Up"),
